...
function getDokterBySpecialtyCode(specialtyCode) {
    ...
    fetch(`/dokter/${specialtyCode}`)
        ...
        .then((data) => {
            ...
            const doctor_list = ScheduleRoutine.reduce((acc, item) => {
                const existing = acc.find((entry) => entry.nip === item.nip);

                if (existing) {
                    existing.jadwal.hari.push(days[item.DayNumber - 1]);
                    existing.jadwal.jam.push(item.OperationalTimeName);
                } else {
                    acc.push({
            ...
            // let jadwalHarian = ScheduleRoutine.map((doctor) => ({
            //     nama: doctor.ParamedicName,
            //     nip: doctor.ParamedicCode,
            //     unitCode: doctor.ServiceUnitCode,
            //     unitName: doctor.ServiceUnitName
