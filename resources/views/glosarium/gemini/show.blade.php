            ...
            <div class="container">
                ...
                <nav class="hero-breadcrumb" aria-label="breadcrumb">
                    <ol class="breadcrumb flex-wrap">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('gemini.index') }}">Kamus Medis</a></li>
                ...
                <div class="card shadow-sm border-0">
                    <div class="card-body p-4 p-md-5">
                        ...
                        <div class="deskripsi-text fs-5 text-secondary" style="line-height: 1.8;">
                            @if(isset($item['details']['sections']) && count($item['details']['sections']) > 0)
                                @foreach($item['details']['sections'] as $section)
                                    <h3 class="mt-4 fw-bold text-dark fs-4">{{ $section['judul'] }}</h3>
                                    
                                    @if(isset($section['paragraf']) && count($section['paragraf']) > 0)
                                        @foreach($section['paragraf'] as $paragraph)
                                            <p class="mb-3">{{ $paragraph }}</p>
                                        @endforeach
                                    @endif

                                    @if(isset($section['list_groups']) && count($section['list_groups']) > 0)
                                        @foreach($section['list_groups'] as $listGroup)
                                            @if(isset($listGroup['judul_list']) && $listGroup['judul_list'] !== '')
                                                <p class="fw-semibold text-dark mt-3 mb-2">{{ $listGroup['judul_list'] }}</p>
                                            @endif
                                            
                                            @if(isset($listGroup['items']) && count($listGroup['items']) > 0)
                                                <ul class="mb-4">
                                ...
                                @endforeach
                            @else
                                <p>Deskripsi tidak tersedia untuk istilah ini.</p>
                            @endif
                        </div>
                        
                        <div class="mt-5">
                            <a href="{{ route('gemini.index') }}" class="btn btn-outline-primary">
